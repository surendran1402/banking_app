...
const Dashboard = () => {
  ...
  const [hideBalance, setHideBalance] = useState(false); // Toggle to hide/show balances
  // Load user's bank accounts and balance
  // Helper to generate local insights when API fails
  const generateLocalInsights = (transactions, currentBalance) => {
    const suggestions = [];
    const now = new Date();
    ...
    // Use 'active' transactions for income (received or deposit)
    const income = thisMonthTx
      .filter(t => (t.direction === 'received' || t.transaction_type === 'deposit'))
      .reduce((sum, t) => sum + (Number(t.amount) || 0), 0);

    // Use 'active' transactions for expense (sent or transfer)
    const expense = thisMonthTx
      .filter(t => (t.direction === 'sent' || t.transaction_type === 'transfer' || t.transaction_type === 'instant'))
      .reduce((sum, t) => sum + (Number(t.amount) || 0), 0);

    const usagePct = income > 0 ? (expense / income) * 100 : 0;

    const summary = {
      total_balance: currentBalance,
      remaining_balance: currentBalance,
      balance_usage_percentage: Math.min(usagePct, 100)
    };
    ...
    if (income > expense * 1.2) {
      suggestions.push({
        ...
        detail: `You've saved ₹${(income - expense).toFixed(0)} this month! Consider moving it to a savings goal.`
      });
    }

    // 2. High Spending Category
    const categoryTotals = {};
    const sentTx = transactions.filter(t => t.direction === 'sent');
    sentTx.forEach(tx => {
    ...
    const totalSpent = sentTx.reduce((sum, t) => sum + (Number(t.amount) || 0), 0);

    if (maxCat && totalSpent > 0 && (maxAmt / totalSpent) > 0.4) {
      suggestions.push({
        priority: 'high',
        emoji: '📊',
        title: `High Spending in ${maxCat}`,
        detail: `You spend ${(maxAmt / totalSpent * 100).toFixed(0)}% of your outgoing money on ${maxCat}.`
      });
    }
    ...
    if (currentBalance < 2000) {
      suggestions.push({
        ...
        detail: `Your balance is ₹${currentBalance}. Avoid unnecessary expenses.`
      });
    }
    ...
    if (suggestions.length === 0) {
      if (transactions.length === 0) {
        suggestions.push({
          ...
          emoji: '👋',
          title: 'Welcome!',
          detail: 'Start transacting to see personalized insights here.'
        });
      } else {
        suggestions.push({
          priority: 'low',
          emoji: '👍',
          title: 'All Good',
          detail: 'Your financial health looks good. Keep it up!'
        });
      }
    }
    ...
    const loadUserData = async () => {
      try {
        ...
        if (accountsResponse.success) {
          setBankAccounts(accountsResponse.bank_accounts || []);
        }

        // Get balance
        const balanceResponse = await bankingAPI.getBalance(token);
        if (balanceResponse.success) {
          currentBalance = balanceResponse.total_balance || 0;
          setTotalBalance(currentBalance);
        }

        // Get recent transactions
        const txResponse = await bankingAPI.getTransactions(1, 100, token);
        if (txResponse.success) {
          fetchedTransactions = txResponse.transactions || [];
          // Show all relevant transactions for the user
          setRecentTransactions(fetchedTransactions.slice(0, 5));

          // Calculate category spending from sent transactions only for the chart
          const spending = {};
          fetchedTransactions.filter(tx => tx.direction === 'sent').forEach(tx => {
            const cat = tx.category || 'Other';
            spending[cat] = (spending[cat] || 0) + (Number(tx.amount) || 0);
          });
          if (Object.keys(spending).length > 0) {
            setCategorySpend(spending);
          }
        }

        // Personalized insights (optional - may not be available)
        setAiLoading(true);
        try {
          const insights = await bankingAPI.getAIInsights(token, selectedAccountId !== 'all' ? selectedAccountId : undefined);
          if (insights.success && insights.suggestions && insights.suggestions.length > 0) {
            setAiSuggestions(insights.suggestions || []);
            if (insights.category_spend && Object.keys(insights.category_spend).length > 0) {
          ...
          // AI insights not available - use transaction-based spending calculated above
          console.log('AI insights not available, generating local insights');
          const { suggestions, summary } = generateLocalInsights(fetchedTransactions, currentBalance);
  ...
  const refreshUserData = async () => {
    try {
      ...
      if (txResponse.success) {
        fetchedTransactions = txResponse.transactions || [];
        // Show all relevant transactions
        setRecentTransactions(fetchedTransactions.slice(0, 5));

        // Calculate category spending from sent transactions only
        const spending = {};
        fetchedTransactions.filter(tx => tx.direction === 'sent').forEach(tx => {
          const cat = tx.category || 'Other';
          spending[cat] = (spending[cat] || 0) + (Number(tx.amount) || 0);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Welcome Back, {user.name.charAt(0).toUpperCase() + user.name.slice(1,)}</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 relative transition-all duration-300 transform-gpu hover:-translate-y-1 hover:shadow-lg hover:z-10">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900 mb-1">
                {loading ? (
                  <span className="inline-block animate-pulse bg-gray-200 h-8 w-32 rounded"></span>
                ) : hideBalance ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 relative transition-all duration-300 transform-gpu hover:-translate-y-1 hover:shadow-lg hover:z-10">
            ...
            <div>
              {hideBalance ? (
                <span className="text-2xl font-bold text-green-600 mb-1">₹ ••••••</span>
              ) : (
            ...
            {aiLoading ? (
              ...
              <div className="space-y-3">
                {aiSuggestions.slice(0, 3).map((s, idx) => (
                  <div key={idx} className={`p-3 rounded-lg border-l-4 ${s.priority === 'high'
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left py-4 px-4 text-sm font-semibold text-gray-900">From</th>
                    <th className="text-left py-4 px-4 text-sm font-semibold text-gray-900">To</th>
                    <th className="text-left py-4 px-4 text-sm font-semibold text-gray-900">Transaction ID</th>
                    ...
                    <tr key={t.id || t.transaction_id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <div className="flex items-center space-x-3">
                          <div className="w-8 h-8 rounded-full flex items-center justify-center bg-blue-100">
                            <span className="text-blue-600 font-bold text-xs">
                              {(t.sender_name || 'U').charAt(0).toUpperCase()}
                            </span>
                          </div>
                          <div>
                      ...
                      <td className="py-4 px-4">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{t.category || 'Transfer'}</p>
                        </div>
                      </td>
                      <td className="py-4 px-4">
                        <span className="text-sm font-mono text-gray-600">
                          {(t.transaction_id || `TX${String(index + 1).padStart(6, '0')}`).slice(0, 12).toUpperCase()}
                        </span>
                      </td>
                      <td className="py-4 px-4">
                      ...
                      <td className="py-4 px-4">
                        {hideBalance ? (
                          ...
                          <div className={`text-sm font-semibold ${t.direction === 'sent' ? 'text-red-600' : 'text-green-600'}`}>
                            {t.direction === 'sent' ? '-' : '+'}
                            <CountUp value={Number(t.amount) || 0} currency duration={900} />
                          </div>
                        )}
                      </td>
                      <td className="py-4 px-4">
