...
const TransferProcess = () => {
  ...
  useEffect(() => {
    const fetchUserAccounts = async () => {
      try {
        ...
        if (response.success) {
          const accounts = response.bank_accounts || [];
  ...
  // Helper function to detect if input is a NeoBank account number
  const isAccountNumber = (input) => {
    return /^NB[A-Z0-9]+$/i.test(input);
  };
  ...
  const handlePINVerification = async (pin) => {
    ...
    try {
      ...
      const isEmailInput = isEmail(recipientInput);
      const isAccountInput = isAccountNumber(recipientInput);

      const transferData = {
        recipientPublicId: recipientInfo.public_id,
        recipientAccountNumber: isAccountInput ? recipientInput : recipientInfo.account_number,
        recipientProfileUrl: recipientInfo.profile_url,
        recipientMobileNumber: isMobileNumber(recipientInput) ? recipientInput : null,
        recipientEmail: isEmailInput ? recipientInput : recipientInfo.email,
        amount: parseFloat(amount),
        description,
        category
      };
          ...
          <div className="flex items-center justify-center space-x-4 mb-6">
            {[1, 2, 3, 4].map((step) => (
              <div key={step} className="flex items-center">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${step <= currentStep
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Enter email, account number (NB...), mobile number, or customer ID"
                ...
                <div className="mt-2 text-xs text-gray-500">
                  ...
                  {recipientInput && (
                    <div className="mt-2">
                      ...
                      {isAccountNumber(recipientInput) && (
